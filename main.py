...
def veri_getir(ticker: str, periyot: str) -> pd.DataFrame:
    ...
    veri = binance_klines_getir(ticker, gun_sayisi)
    if veri.empty:
...
tab_tarayici, tab_bot = st.tabs(["🔍 Manuel Tarayıcı", "🤖 Otomatik Bot Durumu"])
...
with tab_bot:
    st.write(
        "Bu sekme, `trading_bot.py` ile ayrı olarak çalıştırdığın otomatik işlem "
        "botunun son durumunu gösterir. Bot bu panelden başlatılmaz/durdurulmaz; "
        "terminalde `python trading_bot.py` ile ayrıca çalıştırman gerekir."
    )

    if not BOT_DURUM_DOSYASI.exists():
        st.info("Bot henüz hiç çalıştırılmamış (durum dosyası bulunamadı).")
    else:
        try:
            durum = json.loads(BOT_DURUM_DOSYASI.read_text())
        except json.JSONDecodeError:
            st.error("Durum dosyası okunamadı.")
        else:
            mod = "🟢 CANLI (gerçek para)" if durum.get("canli_mod") else "🧪 SANAL (dry-run)"
            st.metric("Çalışma Modu", mod)

            dcol1, dcol2, dcol3 = st.columns(3)
            dcol1.metric("Sanal/Gerçek Bakiye (USDT)", f"{durum.get('bakiye_usdt', 0):.2f}")
            dcol2.metric("Açık Pozisyon", len(durum.get("pozisyonlar", {})))
            dcol3.metric("Bugünkü Gerçekleşen K/Z", f"%{durum.get('gun_gerceklesen_pnl_yuzde', 0):.2f}")

            if durum.get("pozisyonlar"):
                st.subheader("Açık Pozisyonlar")
                st.table(
                    [
                        {
                            "Sembol": sembol,
                            "Giriş Fiyatı": format_fiyat(p["giris_fiyati"]),
                            "Miktar": p["miktar"],
                            "Giriş Zamanı": p["giris_zamani"],
                        }
                        for sembol, p in durum["pozisyonlar"].items()
                    ]
                )

            if durum.get("son_islemler"):
                st.subheader("Son İşlemler")
                st.table(durum["son_islemler"][-10:])

            st.caption(f"Son güncelleme: {durum.get('son_guncelleme', 'bilinmiyor')}")

st.markdown("---")
st.caption(
    """
⚠️ **YASAL UYARI:** Bu uygulamada yer alan tüm bilgiler, grafikler, al-sat sinyalleri,
geçmiş test (backtest) sonuçları ve otomatik bot işlemleri tamamen teknik göstergeler
(RSI, hareketli ortalama) baz alınarak otomatik hesaplanmaktadır ve **kesinlikle yatırım
danışmanlığı kapsamında değildir.** Geçmiş performans gelecekteki kazancı garanti etmez;
özellikle SHIB gibi yüksek oynaklığa sahip kripto varlıklarda sermayenizin tamamını
kaybedebilirsiniz. Otomatik bot gerçek modda çalıştırıldığında gerçek parayla emir açar/kapatır
ve doğrudan sorumluluk kullanıcıya aittir. Burada yer alan yorumlar doğrultusunda yapılacak
işlemler sonucunda oluşabilecek zararlardan bu yazılım ve geliştiricisi sorumlu tutulamaz.
Güzel kızıma ithafen yapılmıştır. GRA
"""
)
